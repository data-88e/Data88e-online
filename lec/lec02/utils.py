...
ESG_wo_hun = ESG_table.where("UNIT NAME", are.not_equal_to("HUNTINGTON BEACH 1&2"))

def ESG_plot_wo_hun(price):
    production(price, ESG_wo_hun)
    plt.figure(figsize=(9,6))
    plt.bar(new_x, height, width=width, color=ESG_wo_hun['Color'], edgecolor = "black")
    plt.title('All Energy Sources')
    plt.xlabel('Capacity_MW')
    plt.ylabel('Variable Cost')
    plt.axhline(y=price, color='r', linewidth = 2)
